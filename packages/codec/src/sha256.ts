...
export async function computeSHA256(data: Uint8Array): Promise<string> {
  ...
  // Pure JS fallback algorithm for node/browser legacy contexts
  return syncSHA256Hex(data);
}

export function syncSHA256Hex(data: Uint8Array): string {
  ...
  const bitsHi = Math.floor((l * 8) / 0x100000000);
  const bitsLo = (l * 8) >>> 0;
  view.setUint32(padded.length - 8, bitsHi, false);
  view.setUint32(padded.length - 4, bitsLo, false);
